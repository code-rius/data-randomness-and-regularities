...
\sectionWithoutNumber{Term dictionary}{Term dictionary}{
  \textbf{CNN} - A subset of Neural Networks that include convolutional layers.
  \newline
  \textbf{Microservices} - is a distinctive and organizational approach to software development where the focus is on building single-function modules with well-defined interface and operations. Small, self-contained teams own these modules~\cite{microservices}.
  \newline
  \textbf{Data model} - in the context of machine learning - the final "thing" that results from training a neural network model. It is a collection of layers, and trained weights and biases.
  \newline
  \textbf{Activation function} - in the context of machine learning a function which outputs a small value for small inputs, and a larger value en input exceeding a given threshold~\cite{activation}.
  \newline
  \textbf{Max pooling} - in the context of machine learning, a function to progressively decrease the spatial size of the representation to reduce the amount of parameters and computation in the network~\cite{max-pooling}.
  \newline
  \textbf{JSX} - a syntax extension to JavaScript most commonly used in ReactJs applications.
  \newline
  \textbf{JSON} - a JavaScript Object Notation. A popular data-interexchange format.
}
...
Given a \code{data} array of decimal numbers, the very primitive python implementation of the algorithm would look similar to this:
...
Looking back at listing~\ref{lst:rp-primitive} we can identify that when the first level loop iterator \code{i} is equal to the second level loop iterator \code{j} - the algorithm compares a distance between the same signal states as \code{data[i]} and \code{data[j]} use the identical index.
...
% In this thesis we analyzed the non-triviality of digital signals.
...
The \code{<Selector />} component sends an HTTP GET request to the back end service to retrieve a list of available plot data.
...
\begin{appendices}
  ...
  \newpage
  \section{Calibrating recurrence plot threshold \code{r}}
...
def calibrate_r(input_data, pixel_percentage_target, allowerd_deviation):
  ...
  \begin{center}
    \makebox[\textwidth]{\includegraphics[width=18cm]{assets/website.png}}
  \end{center}
  \label{app:website}
\end{appendices}
